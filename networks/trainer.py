...
class Trainer(BaseModel):
    ...
    def __init__(self, opt):
        super(Trainer, self).__init__(opt)
        self.opt = opt
        # self.model = get_model(opt.arch)  # make sure get_model passes num_heads if you added it
        self.model = get_model(opt.arch, num_heads=opt.num_heads, feature_dropout=opt.feature_dropout)


        # init all heads
        if hasattr(self.model, "fcs"):
            for fc in self.model.fcs:
                torch.nn.init.normal_(fc.weight.data, 0.0, opt.init_gain)
                if fc.bias is not None:
                    nn.init.constant_(fc.bias.data, 0.0)
        else:
            # fallback for old single-head
            torch.nn.init.normal_(self.model.fc.weight.data, 0.0, opt.init_gain)
